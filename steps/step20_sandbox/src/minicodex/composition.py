...
from minicodex.memory import Memory, memory_toolset
from minicodex.memory_write import note_toolset
...
from minicodex.skills import Skills, skill_toolset
...
def local_tools(
    ...
    *,
    ...
    `extra_read_roots` widens only what `read_file` may read (`paths.resolve`),
    never what `apply_patch` may write -- codex's own `helper_readable_roots`
    has the same one-way shape (`core/src/config/mod.rs:3451,4044-4046`).
    Chapter 16 is the only caller, passing the memory directory.
    """
    context = tool_context(
        root=root, session=session, shell=shell, extra_read_roots=extra_read_roots
    )
    return ToolSet(
...
def top_level_tools(
    ...
    *,
    ...
    every session sends before anybody had agreed to be remembered (F17-11),
    and `None` here is what "the user did not ask for this" looks like from
    the composition root. A child gets none for the same reason it gets no
    plan -- and because a sub-agent that could read memory is one more place
    a poisoned entry reaches, with no human anywhere near the transcript.

    `dedicated_tools` is codex's own flag, `memories.dedicated_tools`
    (`config/src/types.rs:299,327,344`, default `false` there too). It has
    nothing to do with whether the memory fits in the resident block -- an
    earlier version of this function gated `memory_toolset` on
    `memory.overflows()`, which is not what codex does and was retired for it
    (F16-12). The default path is `read_file`, already pointed at the memory
    directory by `extra_read_roots` below; `memory_search`/`memory_read` exist
    only when this flag is explicitly true, exactly like codex's.

    `skills` and `skill_tool` are chapter 18's pair of the same shape, and the
    default is the same shape too (F18-14): a skill catalog prints each
    entry's path, `extra_read_roots` makes that path readable, and the model
    opens it with `read_file` -- which is what codex's own instructions tell
    it to do for a `file` entry (`ext/skills/src/catalog_prompt.rs:7`).
    `skill_tool` adds `read_skill` on top. Unlike `dedicated_tools` it has no
    counterpart in codex at all -- codex's `skills.read` serves a kind of
    skill this program cannot have -- and it is kept only because it measured
    better than the default path. See `skills.py`.
    """
    tools = local_tools(
        root,
        session,
    ...
    # not a reintroduction of the overflow gate this replaced (that one asked
    # "did it fit the budget"; this one asks "is there anything to search at
    # all"), and unlike the overflow question this one is not something codex
    # itself would ever get to ask: its `dedicated_tools` flag is resolved
    # once per session, before any memory content is read.
    if memory is not None and memory and dedicated_tools:
        tools = tools.plus(memory_toolset(memory))
    # Chapter 17's one model-facing tool, and the only write access a model has
    # to memory anywhere in this program: it appends a proposal to `notes/` and
    # cannot touch `MEMORY.md` (F17-09).  A separate switch from `memory=`
    # because reading and writing are separate consents -- a user may well want
    # the agent to use what it knows without it recording anything new.
