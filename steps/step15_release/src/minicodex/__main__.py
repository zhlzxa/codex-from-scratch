...
from minicodex.release import apply_deprecations, environment_report
...
async def _ask(
    ...
    *,
    ...
    # different questions -- one re-checks the filesystem every turn, the
    # other speaks exactly once -- so neither replaces the other; both are
    # asked, and whatever either has to say is joined into one developer note.
    # On every turn but the first, at most one of them has anything to say.
...
async def _finish_writer(task: asyncio.Task[Any], *, grace: float = WRITER_GRACE_SECONDS) -> str:
    ...
    if not task.done():
        ...
        # from 3.11 on, and this project supports 3.10, where `wait_for` raises
        # the asyncio one and a bare `except TimeoutError` never fires (F15-02).
...
def _add_permission_flags(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        # Renamed in 0.1.0.  `--yes` still works and warns; `release.py` holds
        # the removal version and a test holds the calendar.  The old name was
        # not merely ugly -- it reads as the answer to the question on screen,
        # and it is the answer to every question this run will ever ask.
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Chapter 5's `minicodex forget N` is not a parser here.  It is rewritten
    # into the line above before argparse sees it, so there is one command with
    # one implementation and the old spelling is a translation at the edge --
    # chapter 1's rule, applied to a command line.  What that buys is the
    # removal: deleting the entry from `release.DEPRECATIONS` in 0.3.0 deletes
    # the whole of it, with no branch left behind to rot.
    ...
    # Not `print_help()` on stdout with exit 0.  Reaching here means the user
    # named no command, which is a usage error, and a program that answers one
    # with "success" plus a menu on stdout is telling every script and CI step
    # that whatever it wanted worked (F15-03).  Found by running the program in
    # an empty directory the way a stranger would, which no test had done.
