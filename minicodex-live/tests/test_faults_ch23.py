...
def test_F23_01_the_public_list_is_short_and_is_all_about_signing_in(anon: TestClient) -> None:
    # The live fork added `/api/health`: process facts for an uptime check.
    # Still four paths, still nothing that names an account.
    assert PUBLIC_PATHS == {
        "/api/auth/status",
        "/api/auth/login",
        "/api/auth/bootstrap",
    ...
    for path in PUBLIC_PATHS:
        method = "GET" if path.endswith(("status", "health")) else "POST"
...
def test_F23_02_the_probe_that_measured_this_is_in_the_step(repo_root: Path) -> None:
    ...
    """
    probe = repo_root / "probes" / "probe_ws_auth.py"
    assert probe.exists()
