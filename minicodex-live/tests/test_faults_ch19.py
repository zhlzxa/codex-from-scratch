...
def test_F19_08_the_seeded_providers_agree_with_the_cli() -> None:
    """Both entry points now import the one table from `model.py`, so this
    asserts the derivation instead of policing two hand-written copies: the
    seed keeps the CLI's `(base_url, model)` pairing, and names stay stable
    ...
    assert seeded == dict(PROVIDERS)
    assert seeded == {
        "ollama": ("http://localhost:11434/v1", "gemma4:31b-cloud"),
        "openai": ("https://api.openai.com/v1", "gpt-4o-mini"),
    }
    names = {p["provider"]: p["name"] for p in SEED_PROVIDERS}
    assert names == {"ollama": "Ollama (local)", "openai": "OpenAI"}
