...
def test_F_1_01_package_is_installed_not_merely_importable() -> None:
    ...
    """
    try:
        # The distribution was renamed minicodex-live when this forked from
        # the tutorial; the import package is still minicodex, so the two
        # names are deliberately different here.
        assert version("minicodex-live")
    except PackageNotFoundError:  # pragma: no cover - only when misconfigured
        pytest.fail("minicodex-live is not installed; run `uv sync --all-extras`")
...
def test_F_1_01_built_wheel_actually_contains_the_data_file(
    ...
    assert "minicodex/web/migrations/env.py" in names
    assert "minicodex/web/migrations/versions/r0001_console.py" in names
    assert "minicodex/web/migrations/script.py.mako" in names
...
def test_F_1_05_every_mutation_script_runs_somewhere(repo_root: Path) -> None:
    ...
    """
    scripts = sorted(f"probes/{p.name}" for p in (repo_root / "probes").glob("probe_mutations*.py"))
