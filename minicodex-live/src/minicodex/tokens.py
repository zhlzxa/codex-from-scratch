...
class UncountableContent(RuntimeError):
    ...
    Per-modality estimation is deliberately not implemented here -- an image's
...
def _content_chars(content: Any) -> int:
    ...
    raise UncountableContent(
        ...
        "only text is modelled. Add a per-modality estimator before "
        "sending this."
    )
...
def estimate_messages(
    ...
    *,
    ...
    budget of a short session.  With MCP tool schemas it is the dominant term.
    """
    chars = 0
