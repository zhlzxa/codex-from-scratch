...
* Concatenating the text into `system_prompt()` puts human-authored content
  behind the same prefix-caching argument `approval.permissions_block`
  already argues for: the block that is *least* likely to
  ...
  by convention (`history.DeveloperNote`'s docstring has the numbers).

  Upstream codex makes the other choice and says so: `AGENTS.md` rides a
  `role: "user"` fragment, re-sent every turn as part of the world-state
  diff, replacing the previous copy.  That fits codex's replaceable-context
  architecture; this program's history is append-only, so the note is said
  once and *changed* by a follow-up note rather than by replacement.  The
  delivery mechanism is the honest consequence of the history model each
  program committed to.

* The history is append-only. A `cd` into a directory with a different
  `AGENTS.md` cannot un-say the one already on record, so it is not silently
  replaced -- a fresh note says plainly that the old one no longer applies,
  the same shape as `rollout.environment_note` for a resumed session whose
  environment moved out from under it.

* `run_shell`'s `cd` is not path-checked (`shell.ShellSession._handle_cd`
  intercepts it for state, not for containment), so an agent's cwd can end up
  outside the sandboxed root the same way a shell can always wander outside
  a directory. Walking upward from *there* looking for a marker would reopen
  exactly that hole: this module never
...
# same way the shell's head/tail clip and compaction's per-message clip were: a
...
def load_project_docs(sandbox_root: Path, cwd: Path, *, max_bytes: int = MAX_BYTES) -> ProjectDocs:
    ...
    the system-prompt module's own docstring) reads as the more specific,
...
class AgentsMdWatcher:
    ...
    as a zero-argument closure -- the same shape as `plan.unfinished_note`
    -- bound to *this run's* shell, so a sub-agent that gets no
    `on_turn_start` at all (an absence, like its missing MCP tools) never
    ...
    def refresh(self, cwd: Path) -> str | None:
        ...
        if not current:
            ...
            # was *not* equal to empty, which means it had content. A dead
            # branch here would be a dead enforcement point.
