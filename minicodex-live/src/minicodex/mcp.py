...
`mcp`, the official Python SDK. codex does the same with the
Rust one (`rmcp = { version = "=3.0.0" }`, `codex-rs/Cargo.toml:393`).

The line this program draws:

    **Hand-roll what you are teaching. Depend on what you are not.**

The registry's concerns were tool-name collisions, a schema budget, footprints, result
normalisation, timeouts, and trusting somebody else's process -- none of which
is message framing. This module adds one more axis: a server that is not a
...
What is this module's own work, and why none of it is the SDK's:
  ...
  - **The environment allowlist.** The SDK ships one
    ...
    at by a real failure -- the plan module's `SYSTEMROOT`/Winsock hang -- and
    keeping it is not stubbornness: an SDK's default is a reasonable guess
    about every caller, and a fault list is a fact about this one.
  ...
  - **Two timeouts, not one.** A server that hangs during startup hangs the
    ...
    different budgets. Remote and local versions of both are
    unequal in practice -- this module's own measurement, not a guess -- see
    docs/history.md.

  - **`call()` never raises for a failed tool.** The loop's rule -- a tool's
  ...
  - **A dead server has to be able to say why.** The SDK reports
    ...
    drained and kept for a subprocess server.

  - **A subprocess server is confined; a remote one cannot be.**  The shell
    runs inside bubblewrap, but confining it alone left a wider door open
    beside it.
    This module closes it for the kind of server it can: `sandbox` reaches
    only the stdio branch below, on purpose. A remote server gets the sandbox's
...
# today's tree; the path drifted from the registry's own citation
# (`rmcp-client/src/rmcp_client.rs`) somewhere between the two crates that now
# both carry an `rmcp_client.rs`, and this module's copy points at the one
...
# Python processes and a pytest that would not exit -- the orphaned-process
# problem again, one dependency later.
SHUTDOWN_TIMEOUT = 15.0

# What an MCP subprocess is allowed to see.  Same allowlist idea as the
# shell's `ENV_ALLOWLIST` and for the same reason -- except an MCP server
...
#
# The SDK has its own list and we do not use it.  `mcp.client.stdio`
...
# and `SYSTEMROOT` is on it because `import asyncio` fails without it
...
#
# Nine variables this module had deliberately left out, arriving because a
...
# "chosen", and the shell allowlist was about choosing.
#
# So `_subprocess_env` blanks them.  A key set to the empty string
...
class RemoteTool:
    ...
    apart is the fix, and keeping the raw one is what makes a call still
    ...
    read-only" and "the server said nothing" are different states, and
...
def _subprocess_env(config: ServerConfig) -> dict[str, str]:
    ...
    The comprehension is the environment allowlist one process further out: the allowlist decides
    ...
    The blanking pass exists only because the SDK unions its
...
def _sandboxed_stdio_params(config: ServerConfig, sandbox: Sandbox) -> StdioServerParameters:
    """`StdioServerParameters` for a subprocess launched inside the sandbox's boundary.

    `sandbox.wrap` takes a shell command string, because the sandbox chose
...
def _as_callback(handler: RequestHandler) -> Any:
    ...
    thing that module knows -- that a server's question goes to the approval
...
class McpClient:
    ...
    def __init__(
        ...
        *,
        ...
        # and the remote-server gap above.
    ...
    async def _target(self, stack: AsyncExitStack) -> Any:
        ...
        the first request goes out, and that is a storage read.

        The stderr detour on the stdio branch exists so failures say why.  `Client` given a
    ...
    def _refresh_last_words(self) -> None:
        ...
        whole of the stderr-detour rule on the live path: when a call fails with `Connection
    ...
    async def start(self) -> None:
        ...
        the SDK does not impose and which the schema budget says has to exist.

        The budget was sized against a subprocess starting up.  It is
        re-measured here against a network round trip instead
    ...
    async def list_resources(self) -> list[Any]:
        ...
        Upstream did not implement this for stdio: a server on your own machine
        ...
        server is not on your own machine, and docs/history.md records the
        measurement of whether a model reaches for it -- the method itself is
        the SDK's `Client.list_resources`.
        """
        client = self._require()
    ...
    def _why(self, exc: Exception) -> str:
        ...
        the traceback that would have said went to a pipe.  The stderr detour is that
        ...
        Dropping `_client` is the other half.  There is no
