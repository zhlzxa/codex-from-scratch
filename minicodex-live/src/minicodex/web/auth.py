...
and a code review that has to notice an *absence*.  This console has
thirty-odd routes; the ones written after the gate existed are
...
PUBLIC_PATHS = frozenset(
    {
        ...
        "/api/auth/login",
        "/api/auth/bootstrap",
        # Process facts only -- version, uptime, turn counters.  No account
        # keys, no thread ids, no workspaces: a monitor is entitled to know
        # the process is alive, and to nothing else.
        "/api/health",
    }
)
...
class AuthContext:
    ...
    def owner(self) -> Owner:
        """The tenancy. The whole reason the `Owner` type exists."""
