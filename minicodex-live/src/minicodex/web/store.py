...
What it *does* borrow is the durability discipline, because that part is not
about concurrency: `_save` writes a temporary file, fsyncs it, renames it
over the target and then fsyncs the directory (see
`persistence.atomic_write_json`).  The rename is atomic on both platforms
this runs on; without the fsyncs it is atomic with respect to *other
processes* and not with respect to power loss.  A settings file that comes
...
from minicodex.model import PROVIDERS
from minicodex.policy import APPROVAL_POLICIES, SANDBOX_MODES

from .persistence import atomic_write_json

#: Display names for the seed entries; the browser renders them, so the
#: canonical spellings stay here rather than being derived from the key.
_PROVIDER_NAMES: dict[str, str] = {"ollama": "Ollama (local)", "openai": "OpenAI"}

#: What the console offers on a fresh install, one entry per provider the
#: CLI knows about.  Derived from `model.PROVIDERS`, so the two entry points
#: cannot disagree about a default model.
SEED_PROVIDERS: tuple[dict[str, Any], ...] = tuple(
    {
        "name": _PROVIDER_NAMES.get(kind, kind),
        "provider": kind,
        "base_url": base_url,
        "model": model,
    }
    for kind, (base_url, model) in PROVIDERS.items()
)

#: Where `minicodex serve` keeps everything it owns: under the working
#: directory's `.minicodex/` like every other file this program writes, or
#: where `MINICODEX_DATA_DIR` points it.  The environment variable is the
#: deployment fork's -- a container keeps state in a mounted volume, and
#: "the cwd of a process started by a service manager" is not a place.
_DATA_DIR_OVERRIDE = os.environ.get("MINICODEX_DATA_DIR", "")
DEFAULT_DATA_DIR = (
    Path(_DATA_DIR_OVERRIDE) if _DATA_DIR_OVERRIDE else Path(".minicodex") / "console"
)
...
#: Memory and skills default **off**: switching either on changes what every
#: later turn sends, and a default-on switch is not a consent. The console
#: asks twice before memory writing, which is the same shape codex's TUI uses
#: for `Enable memories?`.
...
#: The files the single-tenant console wrote straight into its data directory.
...
def adopt_legacy_records(data_dir: Path, owner_key: str) -> list[str]:
    ...
    The single-tenant console wrote `threads.json` and its friends into the
    top of the data directory, because there was one of everybody.  Multi-user
    records live under `tenants/<key>/`, and doing that without moving the old
...
class Store:
    ...
    def _load(self, name: str, default: Any) -> Any:
        ...
        try:
            ...
            # server down: the absence of a thing is not an error.
            return default

    def _save(self, name: str, data: Any) -> None:
        # The shared writer carries the full durability sequence (file fsync,
        # atomic rename, directory fsync) in one place.
        atomic_write_json(self._path(name), data)
