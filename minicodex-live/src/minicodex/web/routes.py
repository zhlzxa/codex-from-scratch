...
from minicodex.rollout import RolloutError, dump_item, fork, list_sessions, read_rollout, resolve
...
from .audit import EVENT_DENIED
...
def _client_host(request: Request) -> str:
    """Who the socket says is calling, or "unknown" when nobody says.

    Deliberately not `X-Forwarded-For`: that header is an assertion made by a
    forwarding proxy, and trusting it when no proxy is configured lets a
    caller write their own source address.  Behind a real proxy the throttle
    would then throttle the proxy -- annoying for the proxy, useless against
    anybody else -- which is the correct failure direction for a default.
    """
    client = request.client
    return client.host if client is not None else "unknown"


def _throttle_check(request: Request, account_key: str) -> None:
    console = _console(request)
    try:
        console.logins.check(account_key, _client_host(request))
    except Throttled as exc:
        # The refusal is the event an operator most needs this file to show.
        console.audit.append(
            EVENT_DENIED, "-", kind="throttled", account=account_key, retry_after=exc.retry_after
        )
        raise HTTPException(429, str(exc), headers={"Retry-After": str(exc.retry_after)}) from exc


def _throttle_failure(request: Request, account_key: str) -> None:
    _console(request).logins.record_failure(account_key, _client_host(request))


def _throttle_success(request: Request, account_key: str) -> None:
    _console(request).logins.record_success(account_key, _client_host(request))
...
def health(request: Request) -> dict[str, Any]:
    """Public, and deliberately anonymous: what an uptime check may see.

    A health endpoint has to be public -- a monitor with no session is still
    entitled to know the process is alive -- but public endpoints answer to
    everybody, so this one answers with process facts and nothing else: is it
    up, how long has it been up, is it doing work.  Even the account question
    is answered as a boolean: whether the bootstrap form is open is a fact an
    anonymous caller needs (the frontend renders from it), while a *count*
    would tell an attacker whether the console has users worth attacking.
    """
    console = _console(request)
    return {
        "ok": True,
        "version": __version__,
        "uptime_seconds": round(time.time() - console.started_at, 1),
        "turns_running": len(console.busy),
        "turns_total": console.turns_total,
        "turns_failed": console.turns_failed,
        "has_account": not console.accounts.empty(),
    }
...
def bootstrap(request: Request, body: Bootstrap) -> JSONResponse:
    ...
    console = _console(request)
    _throttle_check(request, f"bootstrap:{body.key}")
    try:
        account = console.accounts.redeem_bootstrap(body.token, body.key, body.password)
    except AccountError as exc:
        _throttle_failure(request, f"bootstrap:{body.key}")
        console.audit.append("bootstrap", "-", account=body.key, outcome="refused")
        raise HTTPException(400, str(exc)) from exc
    moved = adopt_legacy_records(console.data_dir, account.key)
    if moved:
        log.info("adopted pre-tenancy records into %s: %s", account.key, ", ".join(moved))
    console.audit.append("bootstrap", account.key, outcome="ok")
...
def login(request: Request, body: Credentials) -> JSONResponse:
    ...
    Failures are also charged to the (account, source) pair in the throttle,
    and the check runs *before* `verify` -- a throttled source does not get to
    spend the scrypt cost it is no longer allowed to spend.
    """
    console = _console(request)
    _throttle_check(request, body.key)
    account = console.accounts.verify(body.key, body.password)
    if account is None:
        _throttle_failure(request, body.key)
        # The account *name* is not the actor -- nobody authenticated -- but
        # a trail that hides failed attempts hides the attack.  Recorded with
        # actor "-" and the tried name in `detail`.
        console.audit.append("login", "-", account=body.key, outcome="failed")
        raise HTTPException(401, "that account name and password do not match")
    _throttle_success(request, body.key)
    console.audit.append("login", account.key, outcome="ok")
    return _sign_in(request, account)
...
def _revoke_everything(console: Any, account_key: str) -> dict[str, int]:
    ...
    Cancelling is the same call the stop button makes, so the turn ends the
    way a CLI Ctrl-C does: `mark("interrupted")`, a resumable session file.
    """
...
def _validated_workspace(raw: str, account: Any) -> str:
    ...
    The last check is the one that stops the tenancy from being decorative:
    every other resource this server separates -- records, tokens, memories,
    approvals -- lives under a key.  A workspace does not: it is any absolute
    path on the server, and the entire purpose of the thing being pointed at
    it is to read files and run commands there.  Two accounts with
    unrestricted workspaces are two accounts that can each open a thread on
    the other's home directory, and no amount of tenancy underneath changes
    that.
...
def _within(path: Path, root: Path) -> bool:
    ...
    followed first rather than after -- the same lesson a sandbox's writable
    roots teach.
    """
    try:
...
def create_thread(request: Request, body: NewThread) -> dict[str, Any]:
    ...
    try:
        ...
        if exc.status_code == 403:
            # A workspace refusal is a boundary holding: record which account
            # probed it and where, or the audit trail only shows successes.
            console = _console(request)
            console.audit.append(
                EVENT_DENIED,
                auth.account.key,
                kind="workspace",
                workspace=str(body.workspace)[:200],
            )
        raise
    try:
...
def get_thread(request: Request, thread_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        items = [dump_item(item) for item in history.items]
...
def thread_sessions(request: Request, thread_id: str) -> list[dict[str, Any]]:
    ...
    A thread is a directory of sessions, not one file: every turn writes a
    new one and resumes from the last. This is what the fork menu picks
    from.
    """
    console = _console(request)
    auth = _auth(request)
...
def fork_thread(request: Request, thread_id: str, body: ForkBody) -> dict[str, Any]:
    ...
    `rollout.fork` is unchanged; all this adds is that the copy lands in a
    *different* thread directory, so the original is still there to go back
    to.
    """
    console = _console(request)
    auth = _auth(request)
    ...
    # The CLI resolver accepts arbitrary paths; the web API accepts only
    # references belonging to this thread, including after symlink resolution.
    if not re.fullmatch(r"[A-Za-z0-9_-]+", body.session):
        raise HTTPException(400, "session must be a session id or 'last'")
    try:
        source = (
            resolve("last", source_dir)
            if body.session == "last"
            else source_dir / f"{body.session}.jsonl"
        ).resolve()
        if source.parent != source_dir.resolve():
            raise RolloutError("session is outside this thread")
        if not source.is_file():
            raise RolloutError("no such session in this thread")
    except (RolloutError, OSError, ValueError) as exc:
        raise HTTPException(400, "no such session in this thread") from exc
    ...
    try:
        fork(source, upto=body.upto, directory=store.thread_dir(fresh["id"]))
...
async def send_message(request: Request, thread_id: str, body: SendMessage) -> dict[str, Any]:
    ...
    # Database reads can fail. Resolve configuration before charging or claiming
    # the thread, so an outage cannot strand a busy flag without a running task.
...
def _rules(request: Request, thread_id: str) -> RuleStore:
    ...
    A remembered "always allow `pytest`" is a fact about a checkout, not
    about a person: it lives in `<workspace>/.minicodex/rules.json` so the
    CLI and the console find the same one, and two accounts working in the
    same directory are two people who have agreed the same thing about the
    same repository.  You still have to own the thread to get here at all.
    """
    auth = _auth(request)
...
def get_rules(request: Request, thread_id: str) -> list[dict[str, Any]]:
    ...
    Only project-scoped rules survive a process, so this lists what is on
    disk for the workspace.  A rule nobody can see is a rule nobody can
    revoke.
    """
...
def get_memory(request: Request, thread_id: str) -> dict[str, Any]:
    """The memory this thread's turns actually read.

    Answers with `owner.memories()` -- the only spelling that agrees with
    where `runtime.run_turn` loads memory from.  The two agree by
    construction because both take the same `Owner`.
    """
    auth = _auth(request)
...
def get_skills(request: Request, thread_id: str) -> dict[str, Any]:
    """Real, and workspace-relative -- unlike the memory route above, and
    that is correct: skills stayed per-project when memory moved to the home
    directory, so this is the directory `load_feature_dirs` reads.
    """
    auth = _auth(request)
    record = _console(request).require_thread(auth.owner, thread_id)
...
# -- MCP servers -------------------------------------------------------------
#
# Two shapes behind one record, the same split `mcp.load_config` reads from
# a config file -- but a web form rejecting a bad shape has to become a
# sentence a person reads: a stdio server has a `command`; a remote one has a
# `url` and one of two authentication paths, a bearer token or OAuth, never
# both.
...
# -- MCP OAuth: the console's own architecture ------------------------------
#
# See `web/oauth.py`'s module docstring for why upstream's `redirect_handler`
# /`callback_handler` do not carry over: they assume the agent process and
# the browser share a machine, and a server-hosted console cannot assume
# that.  The callback lands on one of *this console's own* routes instead of
# a loopback port, correlated to a pending attempt by a server-generated
# `state` that expires if nobody ever finishes authorizing.
...
def _oauth_result_page(*, ok: bool, message: str) -> HTMLResponse:
    ...
    return HTMLResponse(_OAUTH_RESULT_PAGE.format(title=title, message=html.escape(message)))
...
async def mcp_oauth_callback(
    ...
    console = _console(request)
    auth = _auth(request)
    # `state` is looked up, never trusted: an unknown, expired or missing
    # value gets the same answer as a value that was never issued at all, and
    # nothing past this line reads anything else the query string says about
