...
This module constructs the `tenancy.Owner` that everything else trusts, and
therefore the thing that has to earn the trust: local accounts, a password, a
session cookie.

**Why not "sign in with GitHub".**  Two reasons, and the first is the one
worth remembering.  OAuth 2.0 is an *authorization* protocol: what a
completed flow gives you is a token that acts on a resource, and "the holder
of this token can read a GitHub repository" is not the same proposition as
"the person in front of this browser is `alice`".  Treating one as the other
is a documented class of bug -- a token minted for any other client, or
lifted from any other application, logs in.  The second reason is smaller and
decisive on its own: GitHub publishes no `registration_endpoint`, so a
GitHub login would require whoever runs this console to register an OAuth App
*before the first person can sign in at all*.  A self-hosted tool that cannot
be started without paperwork on someone else's website is not self-hosted.

**The one thing not hand-rolled.**  Password hashing is the single worst
candidate for a from-scratch implementation in this program.
`hashlib.scrypt` is RFC 7914 in the standard library, so there is no
dependency to add and no KDF to invent.  The code below chooses parameters
and an encoding around it and does no cryptography of its own.
"""
...
#: does not exist so that the miss costs the same as the hit. Built on first
#: use per cost parameter, rather than at import, because it is one full scrypt
#: run and this module is imported by things that never authenticate anybody.
#:
#: Keyed by the cost the hash was built at: a cached dummy built cheaply, then
#: read back under a raised `SCRYPT_N`, makes an unknown-account miss cost
#: *one scrypt at the old price* against a known-account hit at the new one --
#: the exact timing side channel this hash exists to close, reintroduced by
#: the cache. (A timing test pins the invariant.)
_DUMMY: dict[int, str] = {}


def _dummy_hash() -> str:
    if SCRYPT_N not in _DUMMY:
        _DUMMY[SCRYPT_N] = hash_password(secrets.token_urlsafe(32))
    return _DUMMY[SCRYPT_N]
...
class Account:
    ...
    def owner(self, home: Path | None = None) -> Owner:
        ...
        real `~/.minicodex` needs exactly the same hook. `None` keeps the
        default, which is the path the CLI has always used.
        """
...
class AccountStore:
    ...
    and the list of owners cannot live inside one of them; the two also want
    different durability, because a half-written `threads.json` costs a
    session and a half-written `accounts.json` costs everybody the ability to
    log in.
    """
    ...
    #: Guards each read-modify-write cycle over `accounts.json`: the mutating
    #: routes run on FastAPI's worker threads, and two concurrent creators
    #: appending to their own copy of the list would silently drop one
    #: account. An RLock, because `create` is reached through `redeem_bootstrap`
    #: which may already hold it.
    ...
    def _read(self) -> list[dict[str, Any]]:
        ...
        # The same refusal covers a file that parses but is not the array the
        # writer produces -- an object or a bare string is corruption too, and
        # returning `[]` for it would face the same bootstrap race.
        if not isinstance(data, list):
            raise AccountError(
                f"{self.path} does not contain an account list; refusing to treat it as empty"
            )
        return list(data)

    def _write(self, records: list[dict[str, Any]]) -> None:
        # Callers that read-modify-write hold `_lock` across the whole cycle;
        # this helper only does the durable write.
