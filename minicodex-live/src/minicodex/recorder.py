...
**How this differs from the rollout file** (`rollout.py`), which it is easy to
confuse this with: the recorder logs every *request and response* that crosses
the model boundary, including failed attempts and retries -- it is the wire
truth, for replay and divergence checking, with no stability promise.  The
rollout logs the *accepted history items*, one per fact the conversation
keeps, in a versioned format with a migration commitment, and it is what
resume reads.  A 429 shows up here as an attempt record; in the rollout it
shows up as nothing, because a failed attempt produced no accepted item.
"""
...
class Recorder:
    ...
    machine-readable transcript that `replay.py` replays to turn an intermittent
    ...
    def read_all(self) -> list[dict[str, Any]]:
        ...
        document per line means the surviving prefix is still readable; the
