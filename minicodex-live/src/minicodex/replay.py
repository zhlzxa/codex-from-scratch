...
The recorder was built for a human: when an agent misbehaves the first
...
"the recording format replays it".  It could not.  Two things were missing, and both were
...
can be fixed for a recording made before this module.  A loader that quietly
...
What a replay is faithful about, and what it is not:
  ...
  * **not faithful**: the *chunking*.  The recorder stores the assembled turn,
    ...
    fragment assembly cannot be reproduced from a recording; that is
...
# the arrangement that produced this module: two halves of one format, in two
# modules, agreeing until one of them is edited.  The history module put the wire format
...
def call_record(call: ToolCall) -> dict[str, Any]:
    ...
    `raw_arguments` and not the parsed dict, for the same reason the rollout
...
def failure_record(exc: BaseException) -> dict[str, Any]:
    ...
    # because "no status" and "this recording predates the recording format" have to
...
class ReplayDrift(BaseException):
    ...
    and a test double's assertion went into it like everything else.  The
    ...
    This is the every-issued-call-gets-an-output invariant for the third time: a broad `except` is
...
class RecordedAttempt:
    ...
    An *attempt*, not a turn, because the retry loop can make several
...
class Recording:
    ...
    # The rollout learned this once already: `SessionMeta` records cwd, model,
    # sandbox mode and policy "because the history mentions none of them while
    # depending on all of them".  Nothing propagated that to the
    # recorder, so the transcript could not say which
    # model wrote it.  `None` for a recording made before this module.
...
def load(path: Path | str) -> Recording:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            # surviving prefix is still a recording.  The rollout's rule.
            break
        ...
        if kind == "config":
            ...
            # `attempt` arrived with the retry loop.  A recording made
            ...
            # distinction between old formats and old meaning.  Refuse when the
            # information is gone (a tool call recorded without its arguments,
            # below); migrate when it was merely never written down.
            #
            # It was `payload["attempt"]` until an older recording was read
            # by this code for the first time in the field, and
            # produced a bare `KeyError: 'attempt'` traceback.
    ...
    for index, call in enumerate(payload.get("tool_calls", [])):
        if "arguments" not in call:
            raise ReplayError(
                ...
                "the recording format stored only the id and the name."
            )
        calls.append(
            ToolCallDelta(
                call_id=call["id"],
...
def _failure(payload: dict[str, Any], path: Path) -> dict[str, Any]:
    if "status" not in payload:
        raise ReplayError(
            ...
            "a second attempt would see. Recordings made before the recording format cannot "
            "replay a failure."
        )
    return payload
...
class RecordedModel:
    ...
    async def stream(self, messages: Sequence[dict[str, Any]]) -> AsyncIterator[StreamEvent]:
        ...
        if step.prompt_tokens is not None:
            # completion_tokens is not recorded and nothing reads it: compaction
...
class ReplayedTools:
    ...
    *execution* order of two identical calls is the order they were recorded
    in.  The scheduler can put two calls in different batches, so that
