...
decides it is finished.  The loop defined the end of a run as "the model
...
class TaskPlan:
    ...
    the same reason as the approval `Session`, which is the other mutable thing
    in `ToolContext`.  A module-level plan would be shared by every agent in
    the process, including sub-agents.
    ...
    #: that silently changes shape mid-run is a real fault, and the only way to see
...
def _parse(raw: Any) -> tuple[tuple[PlanStep, ...] | None, str | None]:
    ...
    Every rejection is the three-part shape -- what you sent, what is
...
async def update_plan(plan: TaskPlan, args: dict[str, Any]) -> str:
    ...
    # part of the history -- which is the part compaction keeps.
...
def unfinished_note(plan: TaskPlan) -> Callable[[], str | None]:
    ...
    The loop defined the end of a run as "the model asked for no tools this
...
#
# It lives here rather than in `prompts/system.md` for a measured reason:
# prompt that names a tool the current configuration does not have is a prompt
# that gets the model to call something that is not there, measured
