...
For many releases the answer was "the person at the keyboard", and it was
...
The agent is put behind an HTTP server, and the read side has just moved
...
the memory and job paths already use, byte for byte, so the CLI is unchanged and
every test written before this module still describes the program it was
written for.
...
are the account system's job; this module's job is to make sure that when they arrive there
...
# if `..` or an absolute path can travel through it.  The skill catalog
# learned this on a skill name that arrived from the model and closed it
...
# some filesystems is the worst shape this module has to offer: it would
...
class Owner:
    ...
    is what keeps this module backward compatible: `Owner().memories()` is
    ...
    def memories(self) -> Path:
        """The read side's read path, and the write side's write target."""
        return self.root() / "memories"

    def jobs_db(self) -> Path:
        """The memory-write pipeline's claim/lease table.
    ...
    def merge_lock(self) -> Path:
        """The memory-write pipeline's single-writer lock.
    ...
    def mcp_tokens(self) -> Path:
        ...
        *credential* rather than data.  That is a sharper version of the same
        question the tenancy boundary exists to ask: a leaked memory is embarrassing, a leaked
