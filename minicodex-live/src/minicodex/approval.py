...
agreed.  This module is where they meet a human, and it is the *only* place
that happens: `run_shell` and `apply_patch` both go through here, and leaving
no second route to a subprocess is what makes "nothing executes unapproved" a
fact about the code rather than a convention at eight call sites.

The command string is model output crossing a trust boundary, so the gate
runs before the thing it gates is even validated.
"""
...
class Session:
    ...
    Mutable on purpose: `request_upgrade` changes what the rest of the
    conversation may do mid-flight; freezing it would thread a new object
    through every tool handler.  `rules` and `approver` share this lifetime.
    """
...
class ApprovalRequest:
    ...
    reason: str
    risk: Risk
    # The prefix worth offering as a rule, or None when no rule may be made.
    # Computed before the prompt is drawn, so it never offers an option that
...
class Approver(Protocol):
    ...
    A Protocol for the same reason `Model` is one: structural typing means
    the test doubles below inherit nothing.
    """
...
def _looks_like_a_subcommand(word: str) -> bool:
    ...
    Including a flag or path makes the rule too specific, so `pytest -x`
    asks again -- rebuilding the fatigue the rule exists to remove.
    """
...
def _suggested_rule(command: str) -> tuple[str, ...] | None:
    ...
    Program plus up to two subcommand-shaped words -- `("cargo", "test")`,
    not `("cargo",)` (covers every subcommand) and not
    `("cargo", "test", "--all-features")` (matches no next invocation).
    """
    parts = segments(command)
    if parts is None or len(parts) != 1:
        # A rule from the first words of a composition would silently cover
        # whatever the user pipes into it next time.
        return None
...
async def gate_command(command: str, session: Session, *, confined: bool = False) -> GateResult:
    """The one door for shell commands; `confined` says the kernel is helping.

    `confined=True` means the caller has verified via `sandbox.unavailable()`
    that an OS sandbox is actually enforcing the session mode, so
    `policy.judge_command` may consult its confined table (which lets
    `workspace-write` allow writes and interpreters the kernel bounds).
    Callers that do not know default to `False`, which asks instead of
    allowing: the fail-safe direction is one extra prompt, not one
    unconfined write.
    """
    verdict = judge_command(command, mode=session.mode, policy=session.policy, confined=confined)
...
# What changes when an OS sandbox is enforcing the mode: shell commands whose
# reach the kernel bounds stop asking.  `read-only` is unchanged -- the
# confined table adds an interpreter, but any command that *changes* something
# still asks here, so the sentence above stays true.
_MODE_MEANING_CONFINED: dict[SandboxMode, str] = {
    "workspace-write": (
        "you can read files, edit files inside this repository with "
        "`apply_patch`, and run shell commands -- writes and interpreters "
        "included, because the OS sandbox confines every command to this "
        "workspace. Commands that reach the network still need approval."
    ),
}
...
def permissions_block(session: Session, *, can_request: bool = True, confined: bool = False) -> str:
    ...
    `can_request` suppresses the sentence naming `request_permissions` when
    that tool is absent: a prompt that names a tool the model does not have
    is a prompt that spends a turn of the budget teaching it a lie.

    `confined` selects the confined wording: with the kernel enforcing the
    mode, "a shell command that changes anything still needs approval" is no
    longer true and a prompt that says so sends the model to ask for what it
    may already do.
    """
    ...
    if confined:
        mode_meaning = _MODE_MEANING_CONFINED.get(session.mode, mode_meaning)
...
def _denial(what: str, reason: str, session: Session) -> str:
    ...
    The last sentence depends on the policy, and that dependency matters: a
    message that names `request_permissions` where nobody can answer turns the
    sentence into an instruction -- a model told to call it runs it as a shell
    command instead.  Naming a capability is an instruction to use it, so it
    is only named when using it can work.  See `permissions_block` for the
    same rule in the system prompt.
    """
    if session.policy == "never":
        do_this = (
...
class CliApprover:
    ...
    event loop.  Here it is arguably harmless, since there is nothing to do
    while a human decides, but ruff's ASYNC rules do not know that and neither
    will the next reader.  `to_thread` costs one line.
    """
