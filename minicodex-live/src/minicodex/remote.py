...
`mcp.Client` already knows how to speak to a URL -- the registry's line still
...
assumed, because the first draft of this module assumed both and was wrong
about one:
...
# on published ones -- the same rule the registry applied to the SDK's own
...
class RemoteOAuthConfig:
    ...
    divergence measured rather than assumed.
    """
...
class RemoteServerConfig:
    ...
    the same reason `bearer_token_env_var` exists at
...
async def build_oauth_provider(
    ...
    *,
    ...
    a browser or a real port, the same shape the approval gate gave `Session.approver`.
    ...
    and both need to happen before this provider ever sees a request:
      ...
      - `mcp_oauth.preload_tokens`: without it, a token this project
...
async def build_transport(
    ...
    *,
    ...
    `sandbox` never reaches this function, and that asymmetry is the point:
    a subprocess is something this process can confine,
    and somebody else's web server is not. What a remote server gets instead
    is the sandbox's network boundary one level up -- reached from inside
