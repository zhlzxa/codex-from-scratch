...
the `Owner` type the same way `Owner.mcp_tokens()` was already shaped to
expect (`tenancy.py`). The two
...
module driving an injected fake instead -- the same shape the approval gate gave
...
class OwnerTokenStorage:
    ...
    ACLs -- said out loud, the same honesty the tenancy boundary already owes about
    ...
    async def set_tokens(self, tokens: OAuthToken) -> None:
        ...
        # necessary anyway: `OAuthToken.expires_in` is RFC 6749's
...
async def preload_tokens(provider: OAuthClientProvider, storage: OwnerTokenStorage) -> None:
    ...
    This exists because of a real, measured gap rather than as a
