...
The naive reading assumes the first shape and overwrites by index, which against OpenAI
...
#: Default model per provider, with the base URL each defaults to.  Lives here
#: -- beside the URLs it names -- rather than in the two entry points, which
#: had held copies that a test then had to keep in agreement.
...
# whether the parent saw a `timeout` outcome or an exception.  The retry budget makes
...
class Usage:
    ...
    compaction needs it: every local estimate was measured low, always in
...
@dataclass(frozen=True)
class RateLimit:
    """How much of the provider's window is left, as the provider reports it.

    Read off the response headers of a *successful* request, which is the only
    moment it is free.  The retry path already parses these headers, but only out
    of a 429 -- by which time the information is a description of the wall the
    program has just hit.  The background writer needs the same numbers
    before anything goes wrong, because the decision it makes is "should
    this optional work happen at all".

    Everything is optional because not every server speaking this API sends
    any of it: a local ollama sends none, and `headroom()` returning `None`
    means "unknown", which the caller must not read as "plenty".
    """
    ...
    def from_headers(cls, headers: Any) -> RateLimit | None:
        table = {str(k).lower(): str(v) for k, v in dict(headers).items()}

        def number(name: str) -> int | None:
            raw = table.get(name)
            try:
                return int(raw) if raw is not None else None
            except ValueError:  # pragma: no cover - a provider sending "1k"
                return None
        ...
        return value if value.headroom() is not None else None
    ...
    def headroom(self) -> float | None:
        ...
        """
        pairs = [
            (self.remaining_requests, self.limit_requests),
            (self.remaining_tokens, self.limit_tokens),
        ]
        seen = [100.0 * left / total for left, total in pairs if left is not None and total]
...
class ModelHTTPError(RuntimeError):
    ...
    For a long time this was one formatted string, and every caller that
    ...
    Parsed here rather than by whoever catches it: this
...
class ChatCompletionsModel:
    ...
    def __init__(
        ...
        *,
        ...
        # The last successful response's rate-limit headers, or None if the
        # server does not send them.  Mutable state on the client, which is
        # what it is: a fact about the connection rather than about a request.
        # The background writer reads it to decide whether background work may
        # run; nothing in the loop reads it at all.
        ...
        async with httpx.AsyncClient(timeout=self.timeout, transport=self.transport) as client:
            async with client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    # `chunk["choices"][0]` that worked against one provider
