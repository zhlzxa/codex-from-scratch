...
ships.  Everything after it is built on top of it.
"""
...
# Kept in step with pyproject.toml by hand: two places, one number, and the
# packaging test below asserts they agree.  (The version mismatch this guards
# version that never moved; this fork starts its own line at 0.1.0.)
__version__ = "0.1.0"
...
def system_prompt() -> str:
    ...
    Written early with the note "nothing uses this yet" -- it existed
    ...
    broken.  The approval gate gives it a job, because the permission state has to
...
def compaction_prompt() -> str:
    ...
    it is prose that will be edited by someone reading it as prose, and the
