...
# the kept tail.  The shell already clips output at the tool;
...
class Protected:
    ...
    * the **system notes at the front** -- the instructions and the permission
      ...
      different agent, and it matters what one does when it no longer
    ...
    * the **first user message** -- the only record of what was asked.  Cut 6
    ...
    Later system notes are deliberately *not* protected.  The loop's
...
def clip_item(item: HistoryItem, *, max_tokens: int = MAX_ITEM_TOKENS) -> HistoryItem:
    """Shrink a single tool result that is too big to keep whole.
    ...
    Head and tail, like `clip.clip`, and for the same reason:
...
class Sizer:
    ...
    summariser exists to distrust, and a summary made of paths and error strings
...
def plan(
    ...
    *,
    ...
    Deciding the cut and *then* discovering that the summary does not fit
    means handing the messages needed to make a different decision to a model
    and throwing them away.  Reserving is what makes the second pass
...
def _replay(history: History, items: Sequence[HistoryItem]) -> None:
    ...
    This is the whole reason `History` enforced its invariant on append rather
...
def _hard_summary(items: Sequence[HistoryItem], reason: str) -> str:
    """What to say when the summariser could not be reached.
...
async def compact(
    ...
    *,
    ...
    try:
        ...
        # Not every failure of this call deserves the degraded
        ...
        # precisely the case this branch exists for.
    ...
    # The summary is trimmed with the *same* sizer that chose the cut.
...
def make_summariser(model: Any, *, dialect: str = "chat_completions") -> Summariser:
    ...
    land in the conversation it is summarising.  The sub-agent's own run makes this shape
    ...
    async def summarise(request: SummaryRequest) -> str:
        ...
        async for event in model.stream(scratch.to_wire(dialect)):
            if isinstance(event, TextDelta):
                ...
                saw_end = True
        if not saw_end:
            # The same rule as the agent loop: a stream that stopped early is
