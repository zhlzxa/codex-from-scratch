...
`Footprint` and `ToolSet` live here too, by the same rule for the
...
class IncompleteStreamError(RuntimeError):
    ...
    existing tests that import it from there keep working.
    """
...
class ToolCall:
    ...
    the error message can quote it back -- and, for the same reason, so the history
...
class ToolSet:
    ...
    something anybody has to remember.  Drift between tables has already cost
    drifting -- a handler with no schema is never called, a schema with no
    handler produces the "no tool named X" error, which was written for
    ...
    only because the scheduler's default for an unrecognised call is `STATEFUL`.
    ...
    because the registry hands the model client that exact object and
    ...
    # Empty for every set except the one the registry produces, where
    ...
    def plus(self, other: ToolSet) -> ToolSet:
        ...
        A name declared twice is refused rather than resolved.  The registry made
        ...
        how five declared tools become four.

        The new schema list is a new object.  The registry hands the
