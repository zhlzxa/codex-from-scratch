...
The wording in `TOOL_SCHEMAS` is pinned by a snapshot test (see
`test_schemas.py`) because it is load-bearing: the descriptions steer which
tool the model picks, and the one sentence that has never steered anything is
the timeout note -- what redirects the model after a kill is the error
message it receives then, not the promise in the schema.
"""
...
#: A caller that never mentioned the sandbox at all.  Distinct from `None`
#: ("run unwrapped, decided") -- see `tool_context`'s docstring.
_UNSET = object()
...
class ToolContext:
    ...
    `root` decides whether a path is inside the repository, `shell` carries
    the working directory across calls.  Neither belongs to the process,
    which is why neither is a module-level constant.

    `session` is the odd one out: it is mutable, because
    `request_permissions` can change what the rest of the conversation is
    allowed to do.  It sits here rather than being a fourth argument to every
    handler for the same reason the other two do.
    """

    root: Path
    shell: ShellSession
    session: Session
    # Directories `read_file` may also read from, on top of `root`. The
    # memory directory lives outside the repository, so reading it needs a
    # second boundary rather than a wider one. Empty unless a caller grants
    # one, and for a sub-agent, which gets no memory of its own
    # (`composition.py`).
...
class ToolSpec:
    ...
    Handlers and schemas live in one list so they cannot drift: a handler
    with no schema is never called, and a schema with no handler makes the
    model receive the "no tool named X" error written for *hallucinated* tool
    names.
...
async def read_file(root: Path, extra_roots: Sequence[Path], args: dict[str, Any]) -> str:
    """Read a UTF-8 text file from inside the repository, or `extra_roots`.

    `Path.read_text()` blocks, and a blocking call inside `async def` stops
    the whole event loop rather than just this task -- with tools running
    concurrently, the concurrency quietly turns into a queue.  Ruff's
    ASYNC240 rejects the direct call, which is why those rules are enabled.

    `errors="replace"`: a file with one bad byte should come back slightly
    wrong, not as an exception that discards the whole read.

    `paths.resolve()` is where the containment rule actually lives; see it
    for what `extra_roots` buys.
    """
    path, error = resolve(args.get("path"), root, extra_roots=extra_roots)
...
async def run_shell(ctx: ToolContext, args: dict[str, Any]) -> str:
    ...
    # The kernel boundary, if one is in place and able to enforce itself.  A
    # sandbox object that reports `unavailable()` (no bwrap on PATH) degrades
    # to `confined=False`: the gate asks instead of trusting a boundary that
    # is not there.
    sandbox = ctx.shell.sandbox
    confined = sandbox is not None and sandbox.unavailable() is None

    gated = await gate_command(command, ctx.session, confined=confined)
...
async def apply_patch(
    ...
    writing as it goes leaves the repository half-edited when the third of
    five hunks does not match.

    `audit`/`actor` are bound from the shell session by the ToolSpec (the
    console's audited shell carries them; every other caller gets the
    defaults and the patch is not audited -- the core stays silent, the
    server does the recording, the same shape as the sandbox).
    """
    ...
    return await asyncio.to_thread(apply_edits, edits, root, audit, actor)


def footprint_of(call: ToolCall, root: Path) -> Footprint:
    """What one call touches, for the scheduler.
    ...
    from here to there would save that second `stat()`, at the price of every
    tool signature carrying a scheduling detail forever. The two stay
    independent on purpose: the scheduler is allowed to be conservative (and
    re-check) about a call it is not going to run yet, without the tool that
    actually runs it inheriting any of that.
...
def tool_specs(timeout: float = DEFAULT_TIMEOUT) -> list[ToolSpec]:
    ...
    The timeout stays a parameter rather than a number typed into the
    string: there is one number, and a test asserts the sentence matches it.
    """
    return [
        ToolSpec(
            name="read_file",
            ...
            bind=lambda ctx: functools.partial(read_file, ctx.root, ctx.extra_read_roots),
        ),
        ToolSpec(
            name="apply_patch",
            description=(
            ...
            bind=lambda ctx: functools.partial(
                apply_patch,
                ctx.root,
                ctx.session,
                audit=getattr(ctx.shell, "audit", None),
                actor=getattr(ctx.shell, "actor", "-"),
            ),
        ),
        ToolSpec(
            name="run_shell",
...
def tool_context(
    ...
    `shell` exists for the sub-agent path: a child is a second conversation
    that has to start where the first one is standing -- the parent's `cd
    src` lives in a Python attribute and nowhere else.  Passing the object
    rather than the string on purpose: the environment allowlist and the
    timeout are part of "where the parent is" too.

    `sandbox` defaults to a sentinel rather than to `None`, and the difference
    is load-bearing for a server: `None` passed explicitly means "run commands
    unwrapped, I decided", while the sentinel means "this caller never said",
    which for a caller that did not pass `shell` either would build an
    unconfined `ShellSession` and *look* like a decision.  A server that asks
    for a sandbox gets one or gets an error; the sentinel is what keeps a
    forgotten argument from being read as consent.

    When `shell` *is* supplied the sentinel behaves as "adopt what the shell
    carries": the sub-agent path passes a parent shell whose sandbox is part
    of "where the parent is", exactly like its cwd and timeout.  An explicit
    object still wins, so a caller that wants to confine an
    otherwise-unconfined shell can say so without rebuilding it.
    """
    resolved = (root or Path.cwd()).resolve()
    built_shell = shell
    if built_shell is None:
        built_shell = ShellSession(
            cwd=resolved,
            sandbox=None if sandbox is _UNSET else sandbox,
        )
    elif sandbox is not _UNSET:
        # Only reachable through `local_tools(sandbox=...)`: a caller that
        # passes both is stating the boundary outright, not inheriting one.
        built_shell.sandbox = sandbox
    return ToolContext(
        root=resolved,
        # The shell starts where the other tools are pointed: `read_file` and
        # `apply_patch` are bounded by `root`, so `run_shell` bounded by the
        # process would be a difference invisible until somebody points an
        # agent at a directory it was not started in.
        shell=built_shell,
