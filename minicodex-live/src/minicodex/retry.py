...
The probe scripts each needed a retry loop of their own before this
existed:

    # probes/probe_subagent.py
...
-- retries are a designed problem, not a stopgap".  The shipped program had none:
...
**Relation to upstream codex.**  codex classifies with a semantic enum over
its error details (`is_retryable`), which the chat-completions ecosystem
cannot offer directly; the provider `code` string table here is the same
capability in the vocabulary this API surface actually speaks.  codex also
has one more layer this program does not: a transport fallback (WebSocket
back to HTTPS) when a connection dies mid-stream.  Nothing in this program
opens a streaming transport to begin with, so there is nothing to fall back
*from* -- noted so the absence reads as a scope decision, not an oversight.
"""
...
class Failure:
    ...
    # process -- there is no usage chunk on a failure, so compaction's
...
def classify(exc: BaseException) -> Failure:
    ...
    the same rule as unrecognised shell syntax ("unknown asks").  The two
    situations look alike and are not: one has a human at the prompt to
    ...
    if isinstance(exc, IncompleteStreamError):
        ...
        # stream that died halfway left no half-turn anywhere -- so the whole
        # attempt can be made again, byte for byte.
...
def _retry_after(headers: dict[str, str]) -> float | None:
    ...
    RFC 9110 also allows an HTTP date here.  No provider measured in this program
...
class RetryPolicy:
    ...
    for" is an unbounded wait: a sub-agent gets 300 seconds for a
    ...
    The defaults are nested inside the timeouts that already exist:
        ...
        shell command      30s   (shell.DEFAULT_TIMEOUT)
        MCP tool call      60s   (registry)
        one model attempt 120s   (here)
        retry budget       90s
        one sub-task      300s   (subagent.DEFAULT_TASK_TIMEOUT)

    120 + 90 < 300, so a sub-agent's model call can fail, back off and succeed
    inside the deadline its parent gave it.  Before this module the model's
...
def explain(failure: Failure, *, waited: float = 0.0, attempts: int = 1) -> str:
    ...
    The model-facing version of this is measured: an error
    ...
    being dropped with the rest of the headers for a long time.
    """
...
class ModelFailed(RuntimeError):
    ...
    politeness: for a long time the laziest possible thing was what every
