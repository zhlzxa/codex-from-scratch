...
This module writes the memory `memory.py` reads, and it is two stages with a
temporary file between them, because they are two different jobs:

* **Stage 1, per session.**  One finished conversation in, at most a handful
  of durable statements out.  Cheap, parallelisable, and allowed --
  encouraged -- to produce nothing at all.  codex runs its equivalent eight
  at a time (`CONCURRENCY_LIMIT: usize = 8`,
  `memories/write/src/lib.rs:81`); this one runs them one after another,
  because two is the batch size and a pool for two items is a pool for
  nobody.
* **Stage 2, per memory.**  Every pending stage-1 output, plus whatever the
  ...
  merged into the two files the read side reads.  Runs under a lock, once.
...
**The model never writes the memory files.**  It writes stage-1 output, it
can append a note, and that is the whole of its access.  Both of those are
*proposals*; this module is the only thing that opens `MEMORY.md` for
writing, it validates before it does, and it holds a lock while it does.
The rollout's single-writer rule, arriving a third time (see docs/history.md
...
citation count first (`usage.json`), recency second, and age only as a
tie-breaker -- the nine-month-old entry consulted every week is the one a
by-age rule deletes first.

**Nothing here is on by default.**  `--remember` is a flag, the directory is
somewhere a user can see, and one command deletes all of it.
"""
...
# pid, so two writers cannot collide and nothing is ever overwritten --
# append-only discipline, applied to a directory instead of to a file.
NOTES_DIR = "notes"
...
#
# Under `MINICODEX_HOME` rather than under the repository, matching the
# memory directory itself.  It has to: the lock guards writes to
# `~/.minicodex/memories`, which is one
...
# the model reads.
#
# This is the *second* implementation of the rollout's `O_EXCL` lock file
# and it is a deliberate copy, not an extraction.  The rule of three says
# the second occurrence is copied with a note pointing at the first; a third
# site --
...
# How much of one session goes to the extractor.  Sessions are long, the
# useful part of them is not, and the alternative to a bound here is
# compaction all over again in a place with no compaction loop.
TRANSCRIPT_TOKEN_BUDGET = 6000

# How much of one tool result survives into the transcript.  Same clip
# shape as the shell (head and tail): the beginning of a stack trace is
# where the failure is named and the end is where the exit code is.
...
# Shapes that are secret regardless of what they are called.  The
...
def scrub(text: str) -> tuple[str, int]:
    ...
    Applied **twice**, on purpose: once to the
...
class Stage1:
    ...
    The section names follow the structured-summary result (six
...
# The three wordings, measured, weakest first.  All three are here
# rather than only the winner, for the same reason as elsewhere: a claim that one
...
# **STAGE1_QUOTA** -- the careful version, and the one shipped
...
#
# The read side met the same mechanism from the other side, and what it found
...
#
# So both sides agree, and they agree on something worse than "a required
...
def parse_stage1(text: str, *, session_id: str = "") -> Stage1:
    ...
    disagreement -- said no and wrote bullets anyway -- is in docs/history.md.
    """
...
def transcript_of(rollout: Rollout, *, budget: int = TRANSCRIPT_TOKEN_BUDGET) -> tuple[str, int]:
    ...
    block, the budget warning, the `AGENTS.md` injection, the memory block
    itself -- and an extractor shown them
    ...
    looks like an oversight: the `AGENTS.md` note arrives as a developer
    ...
    the three-layer table in `memory.py`.
    """
    parts: list[str] = []
    for item in rollout.items:
        if isinstance(item, UserMessage):
            ...
            if item.name == NOTE_NAME:
                ...
                #
                # A self-conversation must not be remembered as a session, in
    ...
    # Head and tail, the shell-clip shape: the user's actual request is at the top
...
async def extract(
    ...
    *,
    ...
    every failure mode the rest of this program exists to handle, in exchange for
    nothing -- and it would also give a model somewhere to write, which is the
    one thing it must not have: write access to the memory files.
    """
...
async def _ask(model: Any, messages: list[dict[str, Any]]) -> str:
    ...
    The same shape `make_summariser` uses, including the deferred
    ...
    The `[DONE]` check again.  A stream that
...
# the measured trap: a model calling a tool the prompt named
...
def note_toolset(directory: Path, *, limit: int = 5) -> ToolSet:
    ...
    The obvious design -- let the model edit
    ...
    remembers and drops the rest, silently, in a file nobody diffs.  A
    half-applied edit with no reviewer.
...
# ---------------------------------------------------------------------------
# Stage 2: merge everything pending into the two files the read side reads
# ---------------------------------------------------------------------------
...
def prune(
    ...
    *,
    ...
    The fix is "a window of unused days, a total cap, and a periodic prune",
    and taken literally against ranking alone it is wrong: the entry that is
    nine
    ...
    **That primary key is a signal measured as unreliable.**  Two separate
    things went
    wrong with it, and only the first was known when this function was written:

    * The *bias*, measured in the first draft of the counting and inherited whole: a
    ...
    * The *resolution rate*, measured after adopting codex's real
    ...
    The tempting repair is to rank on the other signal instead --
    ...
    # every merge, which makes the git diff work: the merge step needs to know
    # -- useless.
...
def trim_summary(
    ...
    *,
    ...
    """Keep the resident half inside the token cap, deterministically.
    ...
    the reason is the sentence about `Sizer`: **the fault is not that
    ...
    What that marker *means* changed when the write path arrived, and this function
    did not have to.  It used to decide whether the dedicated search tools got
    mounted; that gate was an earlier invention and is gone (
...
def _lock(path: Path = MERGE_LOCK) -> int:
    """The rollout's `O_EXCL` lock, copied deliberately.  See `MERGE_LOCK`."""
...
def write_memory(
    ...
    **validation happens before anything is opened for writing**, which is
    two-phase apply -- validate everything, then write -- at a different scale.  A merge answer
    that parses into no entries at all is refused rather than written, for
    the reason an empty summary is an error: that is not a smaller
...
def _record_first_seen(
    ...
    `usage.json` is the read side's file and this is the write path adding a
...
def ensure_repo(directory: Path) -> bool:
    ...
    build the input its consolidation agent reads.  The reason is not version
...
async def run_pipeline(
    ...
    *,
    ...
    one -- the whole point: a user who has just been told the answer
    ...
    secrets = 0
    # `pending_sessions` reads and parses every rollout file in the sessions
    # directory, and `JobStore` does sqlite I/O.  Both block; this coroutine
    # runs as a background task on a server's event loop, so each blocking
    # step goes to a worker thread -- a slow disk here must not stall the
    # socket reads and turns sharing that loop.
    pending = await asyncio.to_thread(pending_sessions, sessions_dir, exclude=exclude)
    with JobStore(jobs_path) as store:
        store.enrol(pending, now=stamp)
        for job in store.claim(limit=limit, now=stamp):
            try:
                from minicodex.rollout import read_rollout

                rollout = await asyncio.to_thread(read_rollout, job.path)
                stage1 = await extract(model, rollout)
            except Exception as exc:
                # The loop's rule, in a place with no model to hand the error
    ...
    raw = await asyncio.to_thread(pending_raw, directory)
    notes = await asyncio.to_thread(pending_notes, directory)
    ...
    # Every git call below shells out with a 30s timeout; off the loop, all
    # of it, or one slow `git` freezes every socket and running turn.
    await asyncio.to_thread(ensure_repo, directory)
    await asyncio.to_thread(write_gitignore, directory)
    from minicodex.memory import load as load_memory

    current = memory if memory is not None else await asyncio.to_thread(load_memory, directory)
    hand_edits_diff = await asyncio.to_thread(hand_edits, directory)
    merged = await consolidate(
        model, memory=current, raw=raw, notes=notes, hand_edits=hand_edits_diff
    )
    ...
    message = f"memory: {len(raw)} session(s), {len(notes)} note(s)"
    await asyncio.to_thread(commit, directory, message)
