...
What two models actually send when asked to read
...
deterministic work: anything that can be
...
def resolve(
    ...
    caller rather than by this function -- `read_file` passes the
    ...
    The error is always a three-part message, because
    ...
    # Resolve first, judge second. Branching on `is_absolute()` and
