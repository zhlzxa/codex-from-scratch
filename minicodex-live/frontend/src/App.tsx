...
import { api, setUnauthorizedHandler, type PolicyInfo } from "./api";
...
export default function App() {
  ...
  const [view, setView] = useState<"chat" | "extensions" | "account">("chat");
  ...
  // Chapter 23: who is this, before anything else. Every other call on this
  // page is behind the gate, so asking them first would produce a screenful of
  // 401s and then the sign-in form anyway.
  ...
  // A session ends on its own -- twelve hours, or two hours idle -- so a 401
  // from any call is not an error to show, it is the app finding out it is
  // signed out. Registered once, for the life of the page.
  useEffect(() => {
    setUnauthorizedHandler(() => {
      setAuth((prev) => (prev ? { ...prev, signed_in: false, account: null } : prev));
      setThreadId(null);
    });
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (status.signed_in) await loadEverything();
      setBooted(true);
    })().catch(() => setBooted(true));
  }, [loadAuth, loadEverything]);
  ...
  if (!auth || !auth.signed_in) {
    return (
      <SignIn
        status={auth ?? { has_account: true, signed_in: false, account: null }}
        onSignedIn={() =>
          void (async () => {
            const status = await loadAuth();
            if (status.signed_in) await loadEverything();
    ...
    return (
      <div className="app">
        ...
        <main className="main">
          ...
          <AccountPanel status={auth} onSignedOut={() => void loadAuth()} />
        </main>
      </div>
    );
  }
    ...
    <div className="app">
      ...
      <main className="main">
        <div className="mobile-bar">
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => setView("account")}>
            {auth.account?.key}
          </button>
        </div>
