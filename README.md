...
## 这个仓库，和 minicodex-live

两个仓库，回答两个不同的问题。

- **本仓库是推导过程**：27 章、每章一个可独立运行的快照、259 条故障档案（含 27 条
  负结果）。它展示的是怎么想、怎么验、以及哪些猜错了。
- **[minicodex-live] 是交付结果**：从最后一章出发，补上一个能真正部署的服务所需要
  的东西——PostgreSQL 持久化与迁移、登录限流、审计日志、Docker 与 nginx，以及一轮
  完整代码评审后的修复。

想看"怎么一步步长出来的"，读本仓库；想看"最后交付成什么样"，去 minicodex-live。
